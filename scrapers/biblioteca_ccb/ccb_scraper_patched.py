...
class CCBArbitrajeScraper:
    def __init__(self, output_dir: str = "laudos_arbitraje", max_workers: int = 5):
        ...
        file_handler = logging.FileHandler('ccb_scraper.log', encoding='utf-8')
        ...
        self.metadata_file = self.output_dir / "laudos_metadata.csv"
        self.load_progress()
    ...
    def get_page_items(self, page: int = 1, rpp: int = 20, starts_with: str = None) -> Tuple[List[str], int]:
        """
        ...
        timeout_seconds = 60 if rpp > 20 else 30  # Más tiempo para páginas grandes

        try:
            self.logger.debug(f"Solicitando página {page} con rpp={rpp} y filtro={starts_with}")
            response = self.session.get(self.browse_url, params=params, timeout=timeout_seconds)
    ...
    def get_item_bitstreams(self, item_id: str) -> List[Dict]:
        ...
        # Ir directamente a bundles ya que el endpoint de bitstreams no existe
                            ...
                            if bs_response.status_code == 200:
                                ...
                                if '_embedded' in bs_data and 'bitstreams' in bs_data['_embedded']:
                                    for bitstream in bs_data['_embedded']['bitstreams']:
                                        # Obtener información detallada del bitstream
                                        ...
                                        mime_type = 'application/pdf'  # Por defecto para PDFs
                                        size_bytes = 0
    ...
    def download_pdf(self, bitstream_info: Dict, item_metadata: Dict) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                ...
                stream=True, 
                timeout=60,
                allow_redirects=True  # Importante para manejar el 302
            )
            response.raise_for_status()
            ...
            if 'pdf' not in content_type.lower() and not response.content[:4] == b'%PDF':
                ...
                # Continuar de todos modos si el nombre sugiere que es un PDF
    ...
    def process_item(self, item_id: str) -> bool:
        ...
        try:
            ...
            for bitstream in metadata['bitstreams']:
                # Verificar si es un PDF por el nombre si no hay mimeType
                is_pdf = (
    ...
    def run(self, limit: int = None, rpp: int = 20, date_filter: str = None):
        ...
        while True:
            ...
            item_ids, page_total = self.get_page_items(page, rpp, date_filter)

            if page_total > 0 and total_items == 0:
                total_items = page_total
                self.progress['total_items'] = total_items
    
                # — Ajuste del límite SOLO si el usuario lo especificó y excede el total disponible —
        ...
        # Aplicar límite también a items por procesar
        ...
        self.logger.info(f"PDFs guardados en: {self.output_dir / 'pdfs'}")
